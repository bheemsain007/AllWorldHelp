<?php
// trips/edit_activity.php
// Task T25 — Edit an itinerary activity (organizer only; GET pre-fills form, POST updates)
...
$itineraryId = (int) ($_GET['id'] ?? $_POST['itinerary_id'] ?? 0);
...
// ── Trip duration for day selector ───────────────────────────────────────────
$durationDays = 1;
...
$error       = null;
$oldDay      = (int)    $activity['day_number'];
$oldTime     = htmlspecialchars($activity['activity_time']);
$oldTitle    = htmlspecialchars($activity['activity_title']);
$oldLocation = htmlspecialchars($activity['location'] ?? '');
$oldDesc     = htmlspecialchars($activity['description'] ?? '');
$oldCat      = $activity['category'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!CSRF::validateToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($dayNumber < 1 || $dayNumber > $durationDays) {
            ...
            $success = TripItinerary::update($itineraryId, [
            ...
            if ($success) {
                redirect("itinerary.php?trip_id={$tripId}&success=updated");
            } else {
...
<head>
    ...
    <title>Edit Activity — <?php echo $tripTitle; ?></title>
    <style>
...
<div class="container">
    ...
    <div class="trip-banner">
        ✏️ Editing activity for: <strong><?php echo $tripTitle; ?></strong>
    </div>
